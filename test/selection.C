...
//----------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      auto d_leadlep = make_cut(
         ...
         true,"Lead_Lepton_Pt","Lead_Lepton_Pt",80, 0, 600,"p_{T,l}","GeV"
      ...
      auto d_sublep = make_cut(subptfuncstring,
         ...
         true,"Sublead_Lepton_Pt","Sublead_Lepton_Pt",80,0,600,"p_{T,l}","GeV"); //(isolated) lepton sub p_T cut

      auto d_4lep = make_cut("Lepton_n > 3", "four reconstructed leptons|at least four leptons detected in event",
         true,"Lepton_n","Lepton_n",8,0,8,"n_{l}");    
      ...
      auto d_twojet = make_cut("Jet_n > 1","two reconstructed jets|at least two jets detected in event",
         true,"Jet_n","Jet_n",6,0,6,"n_{j}");                                            //two jets
      ...
      auto d_subjet = make_cut(string("Jet_Pt[Jet_DeltaR>=Jet_DeltaR_Min&&Jet_Flav<Jet_Flav_Max].at(1) > ")+to_string(jet_subpt),
            ...
            true,"Sublead_Jet_Pt","Sublead_Jet_Pt",60,0,600,"p_{T,j}","GeV");                                                                       //sublead jet pt
      ...
      auto d_EWKmjj = make_cut(string("Dijet_M > ")+to_string(dijet_mass_loose),
            ...
            true,"Dijet_M","Dijet_M",25,0,2000,"m_{jj}","GeV");                                 //loose dijet mass constraint for electroweak selection
      ...
      auto d_psrapdiff = make_cut(string("Jet12_Eta_Diff > ")+to_string(jet_eta_diff),
            ...
            true,"Jet12_Eta_Diff","Jet12_Eta_Diff",40,0,12,"|#Delta_{jj}|");    //jet psudorapidity difference
      ...
      auto d_VBSmjj = make_cut(string("Dijet_M > ")+to_string(dijet_mass_tight),
            ...
            true,"Dijet_M","Dijet_M_2",25,0,2000,"m_{jj}","GeV");                                 //tight dijet mass constraint for VBS selection
      ...
      h = (TH1F*)d_EWKmjj.Histo1D({"","",10,100,1600},"Dijet_M")->Clone("");
      h->Scale(unweighted_scale);
      output_hists.push_back(h);
      h = (TH1F*)d_VBSmjj.Histo1D({"","",8,400,2000},"Dijet_M")->Clone("");
      h->Scale(unweighted_scale);
      output_hists.push_back(h);
...
/*
   ...
   auto h_ewk = stack_hists("ZZjj Dijet Mass","EWK Selection;m_{jj} [GeV];Events / 150 GeV",output_hists[0],output_hists[2]);
   h_ewk->Draw("hist"); gPad->SetLogy(1); gPad->SaveAs(savefile("EWK_Dijet_M"));
