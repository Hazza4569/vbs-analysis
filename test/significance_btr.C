void significance_btr(string date, string pMethod = "poisson_seq")
{
   auto calcsig = [](double n_s, double n_b, string method = "s_over_root_b",bool verbose=false)
   {
      if (method == "s_over_root_b") return n_s/sqrt(n_b);
   ...
   for (int i=1; i<=mjj_n; i++)
   {
      ...
      for (int j=1; j<=njj_n; j++)
      {
         ...
         double signif = calcsig(n_sig*scale_sig, n_bkg*scale_bkg, pMethod);
